    ...
    public class MarketPlaceFixedTakeProfitStrategy : IStrategy
    {
        ...
        public async Task<bool> Execute(IExchange exchange, INotification notification, ISignal signal)
        {
            ...
            try
            {
                ...
                var order = await Exchange.PlaceOrderAsync(_notification.SymbolName, _notification.SignalPrice, _notification.Signal < 0, false, amount, _notification.StopLoss, takeProfit,
                    signal.Leverage);
                if (!order)
                {
